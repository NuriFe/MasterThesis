%function [PositionErrors,MuscleForces, timer ] = simulate_reaching_movement(G)
    ...
    for indx =1:length(totry)
        ...
        time = 0;
        tend = 2;
        tstep = .001; % default was 0.003; decreased to make more stable numerically
        ...
        while last == 0
            ...
            try
                ...
                tolerance = [0.06 0.06 0.06];
                if i>100 && sstime(indx,:)==0
                    avg_last_50 = mean(PositionErrors(i-50:i-1,:));
                    if all(abs(PositionErrors(i,:) - avg_last_50) < tolerance) 
                        sstime(indx,:)=i*tstep;
                    end
                end
