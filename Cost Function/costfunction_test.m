...
[optimal_position, optimal_cost] = simulannealbnd(@myCostFunction, initial_guess, lb, ub, options);
...
function cost = costfunction(G, data)

    % Target position
    target = data.target;

    % Number of movements
    starts = data.start_poses;
    Nm = length(starts);
    ...
    for movement = 1:Nm
        ...
        [error(movement,:),effort(movement,:)] = simulate_reaching_movement(G);
